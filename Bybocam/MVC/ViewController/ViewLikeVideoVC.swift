...
class ViewLikeVideoVC: UIViewController, UITextFieldDelegate, UITextViewDelegate, UICollectionViewDataSource, UICollectionViewDelegate
{
    ...
    {
        super.viewDidLoad()
        self.collectView.addSubview(refreshControl)
...
//        }
    ...
    func SearchPostApi()
    {
        ...
        ApiHandler.PostModelApiPostMethod(url: LIKED_VIDEO, parameters: para, Header: [ : ]) { (responsedata, error) in
...
extension  ViewLikeVideoVC :UICollectionViewDelegateFlowLayout
{
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize
    {
        
        //  return CGSize(width: (UIScreen.main.bounds.width/2)-18, height: (UIScreen.main.bounds.height/5)-22 )
        return CGSize(width: (UIScreen.main.bounds.width/2)-18, height: 250)
